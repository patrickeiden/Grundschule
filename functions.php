...
function printFormforCustome($file, $checkValue){
  ...
  if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        if($checkValue){
          $form .= '<button type="button" class="btn btn-danger deleteButton2" name="delete" value="'.$a[$i-1].'" formmethod="POST">Löschen</button>';
        }else{
...
function setClasses($uid, $number, $folder){
  ...
  $stmt = $conn->prepare("UPDATE table_data SET classes_file_name=? WHERE user_id=?");
  $stmt->bind_param("si", $folder, $uid);
  $stmt->execute();
